 ...
 */
export function ModelSelect({ value, onChange, filterModality, placeholder, disabled }) {
  ...
  return html`
    <div ref=${wrapperRef} class="relative w-full">
      <input
        ...
        onFocus=${() => { safeSetOpen(true); setQuery(''); }}
        onInput=${(e) => { setQuery(e.target.value); safeSetOpen(true); }}
        onKeyDown=${handleKeyDown}
        disabled=${disabled}
        class="w-full bg-wa-panel text-wa-text px-3 py-2 rounded-lg text-sm border border-wa-border focus:border-wa-teal focus:outline-none disabled:opacity-60"
        title=${value || ''}
      />
      ${open && html`
        <div
